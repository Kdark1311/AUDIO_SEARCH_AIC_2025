"""
ELASTICSEARCH SPEECH RETRIEVAL SYSTEM - FINAL VERSION with INCREMENTAL INDEXING
Compatible with Elasticsearch 8.11.0

Tính năng thêm:
✅ Fuzzy Search (cho phép gõ sai)
✅ Interactive Search Loop (search liên tục cho đến Ctrl+C)
"""
...
class SpeechRetrievalES:
    ...
    def search(self, query: str, k: int = 3, use_fuzzy: bool = False) -> Dict:
        """
        ✨ Tìm kiếm sử dụng keyword và semantic (có thể bật fuzzy)
    ...
    def search_with_frames(self, query: str, k: int = 3, use_fuzzy: bool = False) -> Dict:
        """
        ✨ Tìm kiếm và kèm theo keyframes (có thể bật fuzzy)
...
if __name__ == "__main__":
    ...
    #     use_semantic=True,
    #     load_data=True,  # True = kiểm tra và index file mới
    #     force_reindex=False  # False = bỏ qua file đã index
    # )
    
    # ============ CHẾ ĐỘ 2: CHỈ TÌM KIẾM (Không index) ============
    retrieval = SpeechRetrievalES(
        context_json_dir=AUDIO_DIR,
        base_keyframe_dir=KEYFRAME_DIR,
        host="http://localhost:9200",
    ...
    print("="*80)
    print("\n💡 Hướng dẫn:")
    print("   - Nhập query để tìm kiếm")
    print("   - Thêm 'f:' ở đầu để bật fuzzy search")
    print("   - Nhập 'exit' hoặc Ctrl+C để thoát")
    print("\n📝 Ví dụ:")
    print("   > xe ô tô           (tìm chính xác)")
    print("   > f:oto             (tìm với fuzzy - gõ sai được)")
    print("   > f:hoc sinh        (tìm với fuzzy)")
    print("="*80)
    ...
    try:
        while True:
            ...
            if not query:
                print("⚠️  Query không được để trống!")
                continue
            ...
            # Nhập số kết quả (mặc định 5)
            try:
                k_input = input("📊 Số kết quả (Enter = 5): ").strip()
                k = int(k_input) if k_input else 5
            except ValueError:
                k = 5
            
            # Tìm kiếm
            print(f"\n🔄 Đang tìm kiếm: '{query}' (fuzzy={use_fuzzy}, k={k})...")
            
            try:
                results = retrieval.search_with_frames(query, k=k, use_fuzzy=use_fuzzy)
    ...
    print("\n" + "="*80)
    print("💡 Tips:")
    print("   - Để index lại từ đầu: retrieval.reset_index_tracker()")
    print("="*80)